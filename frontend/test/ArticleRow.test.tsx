...
describe("<ArticleRow>", () => {
  ...
  it("navigates to the article on click", async () => {
    render(<ArticleRow article={makeArticle()} index={0} onToggleSaved={noop} onShare={noop} />);
  ...
  it("shows the excerpt when there is no summary", () => {
    render(<ArticleRow article={makeArticle({ excerpt: "the excerpt" })} index={0}
      onToggleSaved={noop} onShare={noop} />);
  ...
  it("shows a read-more toggle and expands the medium summary", async () => {
    render(<ArticleRow article={makeArticle({ summary_short: "one liner", summary_medium: "the longer one" })}
      index={0} onToggleSaved={noop} onShare={noop} />);
  ...
  it("calls onToggleSaved without navigating", async () => {
    ...
    render(<ArticleRow article={makeArticle()} index={0} onToggleSaved={onToggleSaved} onShare={noop} />);
  ...
  it("shows Unsave when already saved", () => {
    render(<ArticleRow article={makeArticle({ is_saved: true })} index={0} onToggleSaved={noop} onShare={noop} />);
    expect(screen.getByTitle("Unsave")).toBeInTheDocument();
  });

  it("calls onShare", async () => {
    const onShare = vi.fn();
    render(<ArticleRow article={makeArticle()} index={0} onToggleSaved={noop} onShare={onShare} />);
  ...
  it("applies hover background styles for a selected row", async () => {
    render(<ArticleRow article={makeArticle()} selected index={0} onToggleSaved={noop} onShare={noop} />);
